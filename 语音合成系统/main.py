...
class play:
    ...
    def go(self, *arg):
        self.APP_ID = '16442852'
        self.API_KEY = 'IIuYTjdzL8FrO07XMYwxeGtxCCbuN4D4'
        self.SECRET_KEY = 'Kc4HmUwIrl8LKKAByTUpHqe7ZGYZcL54'
        ...
        if self.cb.get() == '请选择-----':
            ...
            self.res = self.client.synthesis(self.tt.get('0.0', 'end'), 'zh', 1,
                                             {'vol': 2, 'spd': 2, 'pit': 3, 'per': 0})
            return self.res
        elif self.cb.get() == '大叔型':
            self.res = self.client.synthesis(self.tt.get('0.0', 'end'), 'zh', 1,
                                             {'vol': 5, 'spd': 7, 'pit': 6, 'per': 1})
            return self.res
        elif self.cb.get() == '精神小伙型':
            self.res = self.client.synthesis(self.tt.get('0.0', 'end'), 'zh', 1,
                                             {'vol': 7, 'spd': 8, 'pit': 8, 'per': 1})
            return self.res
